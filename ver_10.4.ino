...
const String fwVersion = "10.4";  // VERZE S AUTO-PWM PODSVÍCENÍM A FIXY! BT rele Rezim ============================== VERZE =================
...
const char* stationLogosWeb[] = {
   ...
   "https://thequietus.com/app/uploads/2024/05/Pet-Shop-Boys-2022-credit-Erik-Weiss-2-1536x1024.jpg",   // 18
...
void drawVisualizer() {
    ...
    for (int i = 0; i < 16; i++) {
        ...
        if (i <= 5) { // --- BASY A NIŽŠÍ STŘEDY (Sloupce 0 až 5) ---
            ...
            switch(i) {
                case 0: energy = rawVal * 1.3; break;                 // Úderný kopák (čistá okamžitá energie)
                case 1: energy = rawVal * 1.1 + bassMem * 0.2; break;  // Rychlý basový puls
                case 2: energy = rawVal * 0.9 + bassMem * 0.4; break;  // Střední bas
                case 3: energy = rawVal * 0.7 + bassMem * 0.6; break;  // Táhlá basová linka
                case 4: energy = rawVal * 0.6 + midMem * 0.5; break;   // Přechod do nižších středů
                case 5: energy = midMem * 1.1; break;                  // Čisté nižší středy
            }
            energy *= 0.85; // Celkové jemné utlumení basové sekce proti přebuzení
        } else if (i <= 10) { // Středy
            float ratio = (i - 5) / 5.0;
